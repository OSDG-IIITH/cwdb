<script lang="ts">
	import { refreshSource, deleteSource, type Source } from '$lib/api';
	import { Button } from '$lib/components/ui/button';
	import * as Card from '$lib/components/ui/card';
	import { RefreshCw, Github, GitBranch, Clock, Trash2 } from '@lucide/svelte';
...
<Card.Root
	class="group relative flex w-full flex-col justify-between gap-3 rounded-xl border border-border/60 bg-card/40 px-4 py-3 transition-colors hover:border-border md:px-5 md:py-4"
>
		...
		{#if $user && $user.role === 'admin'}
			<div class="flex items-center gap-2">
				<Button
					...
					class="h-7 border-border/60 px-3 text-xs text-muted-foreground hover:bg-background/70"
				>
				...
				<Button
					...
					disabled={refreshing || deleting}
					class="h-7 w-7 border-border/60 hover:bg-destructive/10 hover:text-destructive"
					title="Delete source"
				>
